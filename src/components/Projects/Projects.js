...
const projects = [
  ...
  { number:'04', title:'Budget Buddy', label:'Full-stack product', summary:'A responsive expense-tracking demo that turns a fictional monthly snapshot into practical, resilient financial guidance.', contribution:'Rebuilt the original student prototype around a focused reviewer journey, with typed frontend state, anonymous demo sessions, persistent data, deterministic budgeting insights, and optional AI refinement.', tags:['TypeScript','React Native Web','Flask','PostgreSQL','OpenAI'], url:'https://github.com/toeneeoh/Expense-Tracker', demoUrl:'https://expense-tracker.toeneeoh.workers.dev', image:dashboard, imageAlt:'Budget Buddy financial overview in dark mode' },
];
    ...
    <section className="section" id="work" aria-labelledby="work-title">
      ...
      <div className="project-list">
        {projects.map((project) => <article className="project-card" key={project.title}>
          ...
          <div className="project-info"><div className="project-meta"><span>{project.number}</span><span>{project.label}</span></div><h3>{project.title}</h3><p className="project-summary">{project.summary}</p><p className="project-contribution">{project.contribution}</p><ul className="tag-list" aria-label={`${project.title} technologies`}>{project.tags.map(tag => <li key={tag}>{tag}</li>)}</ul><div className="project-actions">{project.demoUrl && <a className="project-link" href={project.demoUrl} target="_blank" rel="noreferrer">Open live demo <span aria-hidden="true">↗</span></a>}<a className="project-link" href={project.url} target="_blank" rel="noreferrer">{project.linkLabel || 'View source on GitHub'} <span aria-hidden="true">↗</span></a></div></div>
        </article>)}
      </div>
